...
class FlutterHatomplayer {
  ...
  Future<bool> initialize() async {
    ...
    playerId = await _channel.invokeMethod(INIT_PLAYER, arguments);
    if (Platform.isAndroid) {
      textureId = playerId;
    }
    if (playerId != -1) {
      final messageChannel = BasicMessageChannel(
        'flutter_hatomplayer:player@$playerId',
        const StandardMessageCodec(),
      );
      messageChannel.setMessageHandler((message) async {
        debugPrint('on event: $message');
        final mapData = message as Map;
        final event = mapData['event'];
        if (Platform.isIOS) {
  ...
  Future<int?> setPlayConfig(PlayConfig playConfig) async {
    ...
    final Map<String, dynamic> arguments = {'config': playConfig.toJson()};
  ...
  /// [path] 对讲url
  /// [headers] 请求参数
  Future<int?> setVoiceDataSource(
    String? path, {
    Map<String, dynamic>? headers,
  }) async {
    final Map<String, dynamic> arguments = {'path': path, 'headers': headers};
  ...
  Future<dynamic> _sendCmd(String cmd, [Map<String, dynamic>? argument]) {
    ...
    argument['playerHashValue'] = playerId;
